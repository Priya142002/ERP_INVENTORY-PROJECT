            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-50">
                {RECENT_SALES.map((sale) => (
                  <tr key={sale.id} className="hover:bg-slate-50/20 transition-all group">
                    <td className="px-8 py-5">
                      <span className="text-sm font-bold text-slate-900 group-hover:text-blue-600 transition-colors uppercase tracking-tight">{sale.invoice}</span>
                    </td>
                    ...
                    <td className="px-6 py-5 text-right">
                      <span className="text-sm font-mono font-bold text-slate-900">{sale.amount}</span>
                    </td>
                    <td className="px-6 py-5 text-right pr-8">
                      <Badge 
                        variant={sale.status === 'Settled' ? 'success' : sale.status === 'Pending' ? 'warning' : 'info'}
                        className="px-3 py-0.5 text-[10px] font-bold uppercase rounded-md tracking-wider"
                      >
                        {sale.status}
                      </Badge>
                    </td>
